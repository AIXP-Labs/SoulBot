...
Responsibility:
  ...
  2. Deterministically extract the explicit evolution target from user_message
     (absolute-path regex first, then '{name}_aisp' name match).
...
Usage:
    ...
    python resolve_target.py --cache_dir <path> --chosen <aisp_store_program_dir>
        (NEEDS_INTENT_JUDGMENT follow-up: write the upstream-chosen aisp_store
         program as target — tool validates it is inside aisp_store first)
...
Target resolution (2026-06-14 spec):
    ...
    2. No absolute path -> search ONLY aisp_store (NEVER the runner workspace_dir
       or any other directory). Exact '{name}_aisp' match in aisp_store -> use
       (deterministic python strong match). 0 or >1 matches -> emit the aisp_store
...
Exit codes:
    ...
    1 = FAILURE — one of:
          ...
          NO_TARGET_IN_MESSAGE  (nothing parseable AND no aisp_store found)
          CHOSEN_NOT_FOUND      (--chosen path is not an existing directory)
          CHOSEN_OUTSIDE_AISP_STORE (--chosen is not inside aisp_store)
          INDEX_NOT_FOUND       (_index.json missing/unreadable)
    2 = NEEDS_INTENT_JUDGMENT (no abs path + no exact aisp_store name; the
        aisp_store candidate set is returned for upstream LLM intent judgment /
...
# '{name}_aisp' bare-name token (no path separators), e.g. webapp_testing_aisp
_NAME_AISP_RE = re.compile(r'(?<![A-Za-z0-9_\-])([A-Za-z0-9][A-Za-z0-9_\-]*_aisp)(?![A-Za-z0-9_\-])')
...
def _find_aisp_store(workspace_dir):
    """Locate the aisp_store directory via a bounded walk-up from workspace_dir.

    Spec (2026-06-14): when no explicit absolute path is given, the evolution
    target is searched ONLY inside aisp_store — never the runner workspace_dir or
    any other directory. Returns the absolute aisp_store path, or None if none is
    ...
    for _ in range(6):  # bounded walk-up
        if not anc:
            break
        store = os.path.join(anc, "aisp_store")
...
def _enumerate_aisp_store_candidates(aisp_store):
    """List every '{name}_aisp' program in aisp_store with its agent_card
    ...
    """
    out = []
    if not aisp_store or not os.path.isdir(aisp_store):
        return out
    for entry in sorted(os.listdir(aisp_store)):
        d = os.path.join(aisp_store, entry)
        if not (entry.endswith("_aisp") and os.path.isdir(d)):
            continue
        name, desc = "", ""
...
def extract_target_from_message(user_message, workspace_dir):
    ...
    Resolution order (matches B1 v2.30.0 prose, now deterministic):
      ...
      (2) '{name}_aisp' bare name -> scan workspace_dir for '{name}_aisp/'.
    ...
    # (1) Absolute path candidates. Prefer ones that contain '_aisp' or that
    ...
    # fake-path fail-open (residual ②, 2026-06-14). Bare '{name}_aisp' names are
    # handled by the aisp_store search below, not here.
    abs_candidates = []
    ...
    if abs_candidates:
        ...
        # directory on disk. '_aisp'-named existing dirs first, then any existing.
        for cand, aisop_dir in abs_candidates:
            if ('_aisp' in os.path.basename(cand) or '_aisp' in os.path.basename(aisop_dir)) and os.path.isdir(aisop_dir):
    ...
    # ONLY in aisp_store — NEVER the runner workspace_dir or any other directory
    ...
    # scopes intent judgment to the aisp_store program set.
    aisp_store = _find_aisp_store(workspace_dir)
    if not aisp_store:
        return None, None
    # python STRONG match (混合, deterministic first): an exact '{name}_aisp' dir
    # that lives in aisp_store and is named in the message.
    names = []
    for m in _NAME_AISP_RE.finditer(scan_text):
        nm = m.group(1)
        if nm not in names:
            names.append(nm)
    matched = [nm for nm in names if os.path.isdir(os.path.join(aisp_store, nm))]
    if len(matched) == 1:
        return os.path.abspath(os.path.join(aisp_store, matched[0])), "name_scan"
    # 0 or >1 exact matches -> NO deterministic strong match. Defer to upstream
    # LLM intent judgment over the aisp_store candidate set (main() emits
...
def main():
    ...
    parser.add_argument(
        "--chosen",
        default=None,
        help="Absolute path of the aisp_store program selected by UPSTREAM intent "
             ...
             "aisp_store (NEVER the runner / any other dir), runs the sovereignty "
    ...
    # aisp_store (never the runner / any other dir) before writing.
    if args.chosen:
        ...
        if not os.path.isdir(chosen):
            ...
            print(json.dumps(report, indent=2, ensure_ascii=False))
            sys.exit(1)
        store_rp = _realpath(aisp_store) if aisp_store else ""
        chosen_rp = _realpath(chosen)
        if not store_rp or not (chosen_rp == store_rp or chosen_rp.startswith(store_rp + os.sep)):
            report["status"] = "CHOSEN_OUTSIDE_AISP_STORE"
            report["error"] = (
                "--chosen must be a program directory INSIDE aisp_store "
    ...
    if resolved_dir is None:
        aisp_store = _find_aisp_store(workspace_dir)
        if not aisp_store:
            # QR2 (2026-06-15): no absolute path AND no aisp_store directory found
            ...
            report["error"] = (
                "No absolute path operand and no aisp_store directory found to "
                "search — cannot resolve an evolution target. NEVER default to the "
                "runner — escalate to user."
            )
            print(json.dumps(report, indent=2, ensure_ascii=False))
            sys.exit(1)
        # aisp_store exists but no absolute path + no deterministic strong name
        # match -> hand the aisp_store candidate set to upstream LLM intent
        # judgment (混合: python strong-match first, LLM semantic when none;
        # confidence > 80% -> use, <= 80% -> find related programs and ask user).
        # Candidates come ONLY from aisp_store. NEVER default to the runner.
        report["status"] = "NEEDS_INTENT_JUDGMENT"
        report["aisp_store"] = aisp_store
        report["candidates"] = _enumerate_aisp_store_candidates(aisp_store)
        report["error"] = (
            "No absolute path and no exact '{name}_aisp' match. Deferring to upstream "
            "intent judgment over aisp_store candidates (confidence > 80% -> use; "
    ...
    # (explicit_abs_path or aisp_store name_scan) — no further existence check.
